...
// Id of the table
$table['table-name'] = 'medical-record-pats-perms-table';
...
echo '<form method="post" action="'.$form_action.'" id="med_rec_pats_perm_form">';
...
for ($i = 0; $i < count($table['tuples']); $i++) {
	...
	$table['tuples'][$i]['actions'] =
		'<input type="checkbox" '.$checked.' name="pats[]" value="'.$table['tuples'][$i]['account_id'].'" />';
}
...
echo 	'<br />';
echo 	'<p>';
echo		'<input type="hidden" name="change" value="patients" />';
